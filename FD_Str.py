...
def run(): # UI
    st.title("Online Face-Detection System")
    ...
    if choice == 'Image':
        st.markdown(
            '''<h4 style='color: #d73b5c;'>HaarCascade Classifier</h4>''',
        ...
        if img_file is not None:
            ...
            st.markdown(
                '''<h4 style='color: #d73b5c;'>Change following params to get better accuracy.</h4>''',
                unsafe_allow_html=True)
            ...
            if count == 0:
                ...
                st.success("Total faces : " + str(count))
